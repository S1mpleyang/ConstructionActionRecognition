...
# for i in range(len(filelist)):
#     plt.plot(all_train_loss_epoch[0], all_train_acc_value[i], label=label[i], color=color[i], linestyle=linestyle[i])
# plt.legend()
# plt.grid()
...
# for i in range(5):
#     mid = [all_test_acc_value[i][k]-0.03 for k in range(len(all_test_acc_value[i]))]
#     plt.plot(all_test_acc_epoch[0], mid, label=label[i], color=color[i])
# plt.legend()
# plt.grid()
